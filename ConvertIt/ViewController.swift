...
//  ConvertIt
//
//  Created by Marc Marlotte on 10/6/17.
...
class ViewController: UIViewController {
    ...
    let formulasArray = [Formula(conversionString: "miles to kilometers", formula: {$0 / 0.62137}),
                         ...
                         Formula(conversionString: "yards to meters", formula: {$0 / 1.0935}),
                         Formula(conversionString: "meters to feet", formula: {$0 * 3.2808}),
                         Formula(conversionString: "meters to yards", formula: {$0 * 1.0935}),
                         Formula(conversionString: "inches to cm", formula: {$0 / 0.3937}),
                         Formula(conversionString: "cm to inches", formula: {$0 * 0.3937}),
                         Formula(conversionString: "farenheit to celcius", formula: {($0 - 32) * (5/9)}),
                         Formula(conversionString: "celcius to farenheit", formula: {($0 * (9/5)) + 32}),
    ...
    @IBAction func decimalSelecter(_ sender: UISegmentedControl) {
        calculateConversion()
    }
